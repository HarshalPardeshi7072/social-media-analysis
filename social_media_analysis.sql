...
-- Q.1 display location and ID of users.
       select user_id,location from post ;
       
-- Q.2 display Id of users from  maharshtra.
	 select user_id,location from post where location in ('maharashtra');
     
-- Q.3 display average post per user.
       select round(count(post_id) / count(distinct user_id),1) as 'average post' from post ;

-- Q.4  display users with longest caption.
        select user_id,caption, length(caption) as 'caption length' from post order by 3 desc limit 10;
 
-- Q.5  display users with no (0) comments. 
        select * from comments;
        select * from users; 
        SELECT USER_ID, USERNAME FROM USERS WHERE USER_ID NOT IN (SELECT USER_ID FROM COMMENTS) ;

-- Q.6  display user id who posted 5 or more than 5.
        select user_id ,count(user_id) as post_count from post group by 1 having post_count >= 5 order by 2 desc ;
        
-- Q.7  display display users not followed by anyone.
		select * from users ; 
		select * from follows ; 
        select user_id, username as 'users not followed by anyone' from users 
		where user_id not in  (select followee_id from follows) ; 
        
-- Q.8  display users with maximum posts.
		select * from users ; 
        select * from post ; 
        select user_id, count(user_id)as 'post count'  from post 
		group by 1 order by 2 desc; 

-- Q.9  display users with maximum followers.
         select * from follows ;
         select followee_id , count(*) as 'follower count' from follows
         group by 1 order by 2 desc; 

-- Q.10  display users with more than 40 followers.
	     select followee_id , count(*) as 'follower count' from follows
         group by 1  having count(*) > 40 order by 2 desc ;
         
 -- Q.11  display no. of comments by each users. 
		 SELECT user_id, COUNT(*) AS num_comments FROM comments GROUP BY user_id;

-- Q.12   display no. of photos posted by each user.
         SELECT post_id , COUNT(*) AS num_photos FROM photos GROUP BY post_id ;
 
-- Q.13   display videos posted by each user..
         select * from videos;
         SELECT post_id, COUNT(*) AS num_videos FROM videos GROUP BY post_id;
  
-- Q.14   display most used hashtags.
         select * from hashtags ;
         
         SELECT hashtag_id, hashtag_name, COUNT(*) AS num_occurrences
         FROM hashtags
         GROUP BY  hashtag_id
